...
from zouyu.tasks.amp.ampmotion_loader import MotionLoader
from zouyu.tasks.amp.mdp.terminations import DelayedTerminationManager
...
class MotionResetManager:
    ...
    def init(
        ...
        print(
            f"[MotionResetManager] Loaded {len(loader.motion_data)} clips, {motion_count} frames from {motion_dir}"
        )
        ...
        if loader.motion_data_recovery:
            ...
            print(
                f"[MotionResetManager] Loaded {len(loader.motion_data_recovery)} recovery clips, {recovery_count} frames from {recovery_dir}"
            )
    ...
    def reset(
        ...
        if len(delay_ids) > 0:
            ...
            frames = (
                recovery if recovery is not None else self.walk_run_frames[motion_dir]
            )
