...
本指南介绍在 Isaac Lab 中，为强化学习（RL）环境构建灵活课程（curricula）时常用的课程辅助函数与术语。这些工具可以传递给 `isaaclab.managers.CurriculumTermCfg` 对象，从而在训练过程中动态修改奖励项权重与环境参数。

> **注意**
>
> 本指南涵盖三个工具：
> - 用于修改奖励的简单函数： `modify_reward_weight`

> - 用于修改任意环境参数的 term： `modify_env_param`

> - 用于修改 term_cfg 的 term： `modify_term_cfg`

<details>
<summary>课程工具的完整源码</summary>
...
函数 `modify_reward_weight` 会在仿真运行到指定步数之后，更新某个奖励项（reward term）的权重。它可以直接作为 `CurriculumTermCfg` 中的 `func` 传入。
...
类 `modify_env_param` 是 `isaaclab.managers.ManagerTermBase` 的子类，它允许你定位环境中的任意“点分属性路径”（dotted attribute path），并在运行时应用用户提供的函数来计算新值。

它支持嵌套属性、字典键访问、列表或元组下标索引，并在不希望更新时尊重 `NO_CHANGE` 哨兵值。
...
子类 `modify_term_cfg` 提供了更简洁的地址语法风格（与 hydra 配置语法保持一致）。除此之外，它的行为与 `modify_env_param` 完全相同。
