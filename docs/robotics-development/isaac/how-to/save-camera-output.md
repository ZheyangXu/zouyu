...
本指南配套脚本为 `IsaacLab/scripts/tutorials/04_sensors` 目录下的 `run_usd_camera.py` 。

<details>
<summary>run_usd_camera.py 的代码</summary>
...
为了保存相机输出，我们使用 Omniverse Replicator 中的基础 writer 类。该类允许我们以 NumPy 格式保存图像。关于 Basic Writer 的更多信息，请参考 [documentation](https://docs.omniverse.nvidia.com/extensions/latest/ext_replicator/writer_examples.html)。
...
在仿真器逐步推进的过程中，可以将图像保存到指定的文件夹。由于 BasicWriter 只支持使用 NumPy 格式保存数据，我们需要先将 PyTorch 传感器输出转换为 NumPy 数组，然后再将其打包到一个字典中。
...
完成上述步骤后，我们就可以使用 BasicWriter 保存图像。
...
我们提供了一些工具，用于将深度图像投影到 3D 空间。重投影操作使用 PyTorch 运算完成，从而实现更快的计算。
...
另外，也可以使用 `isaaclab.sensors.camera.utils.create_pointcloud_from_depth` 函数，从深度图像创建点云，并将其转换到世界坐标系下。
...
生成的点云可以使用 Isaac Sim 的 `isaacsim.util.debug_draw` 扩展进行可视化。这使得在 3D 空间中可视化点云变得更容易。
...
仿真启动后，你可以观察到不同物体下落。 `IsaacLab/scripts/tutorials/04_sensors` 目录下会创建一个输出文件夹，用于保存图像。此外，你应该能在视口中看到绘制在 3D 空间中的点云。
