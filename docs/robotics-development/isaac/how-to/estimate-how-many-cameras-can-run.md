...
目前在 Isaac Lab 中，存在多种相机类型：USD Cameras（标准相机）、Tiled Cameras（分块相机）以及 Ray Caster cameras（射线投射相机）。这些相机类型在功能与性能上有所差异。 `benchmark_cameras.py` 脚本可用于理解不同相机类型之间的差别，并在相机数量、图像尺寸和数据类型等不同参数设置下，对其相对性能进行表征。

提供该工具的目的，是帮助你在满足具体场景需求的前提下，更容易找到性能最优的相机类型/参数组合。该工具也可以在你希望“在最小化单步耗时的同时，尽可能增加环境数量”的假设下，估算在现实中系统能够运行的最大相机数量。

该工具可以将相机注入到 gym 注册表中的现有任务中，这在特定场景下进行相机基准测试时会很有用。另外，如果你安装了 `pynvml` ，该工具还能在你设定的系统资源利用率阈值之内，自动寻找你的任务环境中可运行的最大相机数量（不进行训练；每个时间步均执行零动作）。

本指南配套脚本为 `scripts/benchmarks` 目录下的 `benchmark_cameras.py` 。

<details>
<summary>benchmark_cameras.py 的代码</summary>
...
关于自动确定最大相机数量的更多信息，请查看与 `autotune` 相关的命令行参数。

## 在任务环境中对比性能，并自动确定任务可用的最大相机数量

目前，分块相机（tiled cameras）是在需要处理多个动态物体时性能最强的相机。

例如，如果要查看你的系统在 cartpole 环境中能否承受 100 个分块相机，并设置每个环境 2 个相机（因此总共 50 个环境），且仅使用 RGB 模式，可以运行：
...
如果你安装了 pynvml（ `./isaaclab.sh -p -m pip install pynvml` ），还可以在给定环境中，根据某个性能阈值寻找你能够运行的最大相机数量。该阈值由以下参数指定：最大 CPU 利用率百分比、最大 RAM 利用率百分比、最大 GPU 计算利用率百分比、最大 GPU 显存利用率百分比。比如，要在 cartpole 中寻找你可运行的最大相机数量，可以运行：
...
Autotune 可能导致程序崩溃，这意味着它曾尝试一次性运行过多相机。不过，max percentage utilization 参数的设计目的正是为了尽量避免这种情况发生。

基准测试的输出不包含训练网络本身的开销，因此你应当考虑下调最大利用率百分比，以为这部分开销留出余量。最终输出的相机数量是“全部相机的总数”，因此若要得到环境总数，请将输出的相机数量除以“每个环境的相机数量”。

## 对比相机类型与性能（不指定任务）

该工具也可以在没有任务环境的情况下评估性能。例如，要使用 2 个标准相机观察 100 个随机物体，可以运行：
...
如果你的系统由于性能原因无法处理上述设置，进程将被杀掉。建议在运行该脚本时监控 CPU/RAM 利用率与 GPU 利用率，以便了解渲染目标相机所需的资源。在 Ubuntu 上，你可以使用 `htop` 和 `nvtop` 等工具实时监控资源；在 Windows 上，你可以使用任务管理器。

如果你的系统在处理目标相机设置时比较吃力，可以尝试以下方法：

* 切换到 headless 模式（提供 `--headless`）。
* 确保你正在使用 GPU pipeline 而不是 CPU。
* 如果你没有使用分块相机（Tiled Cameras），切换到分块相机。
* 降低相机分辨率。
* 减少每个相机的 data_types 数量。
* 减少相机数量。
* 减少场景中的物体数量。

如果你的系统能够处理所设置的相机数量，终端中将打印时间统计信息。仿真停止后，可以使用 `CTRL+C` 将其关闭。
