...
字段 `mjModel.actuator_lengthrange` 包含可行执行器长度的范围（更准确地说，是执行器传动的长度）。这是模拟肌肉执行器所必需的。这里我们重点关注 `actuator_lengthrange` 的含义以及如何设置它。

与 mjModel 的所有其他字段（这些字段都是精确的物理或几何量）不同， `actuator_lengthrange` 是一个近似值。直观地说，它对应于执行器传动在模型的所有"可行"配置上可以达到的最小和最大长度。然而 MuJoCo 约束是软的，所以原则上任何配置都是可行的。但我们需要一个明确定义的范围用于肌肉建模。有三种方法可以设置这个范围：（1）使用所有执行器中可用的新属性 `lengthrange` 明确提供；（2）从执行器所连接的关节或腱的限制中复制；（3）自动计算，如本节其余部分所解释的。这里有许多选项，通过新的 XML 元素 `lengthrange` 控制。

执行器长度范围的自动计算在编译时完成，结果存储在编译模型的 `mjModel.actuator_lengthrange` 中。如果随后保存模型（无论是 XML 还是 MJB），则在下次加载时不需要重复计算。这很重要，因为对于大型肌肉骨骼模型，计算可能会减慢模型编译器的速度。实际上，我们让编译器多线程化正是为了加速这个操作（不同的执行器在不同线程中并行处理）。

自动计算依赖于修改的物理仿真。对于每个执行器，我们通过执行器的传动施加力（计算最小值时为负，计算最大值时为正），在阻尼状态下推进仿真以避免不稳定性，给它足够的时间稳定并记录结果。这与带有动量的梯度下降相关，实际上我们已经尝试过显式的基于梯度的优化，但问题是不清楚我们应该优化什么目标（考虑到软约束的混合）。通过使用仿真，我们本质上是让物理学告诉我们要优化什么。但请记住，这仍然是一个优化过程，因此它有可能需要调整的参数。我们提供了应该适用于大多数模型的保守默认值，但如果它们不起作用，请使用 `lengthrange` 的属性进行微调。

使用此功能时，记住模型的几何形状很重要。这里的隐含假设是可行的执行器长度确实是有限的。此外，我们不将接触视为限制因素（实际上我们在此仿真中内部禁用接触，以及被动力、重力、摩擦损失和执行器力）。这是因为带有接触的模型可能会纠缠并产生许多局部最小值。因此，执行器应该受到限制，要么是因为模型中定义的关节或腱限制（在此仿真期间启用），要么是由于几何形状。为了说明后者，考虑一个腱，一端连接到世界，另一端连接到围绕连接到世界的铰链关节旋转的物体。在这种情况下，腱的最小和最大长度是明确定义的，取决于附着点在空间中描绘的圆的大小，即使关节和腱都没有用户定义的限制。但如果执行器连接到关节，或连接到等于关节的固定腱，那么它是无限的。在这种情况下编译器将返回错误，但它无法判断错误是由于缺乏收敛还是因为执行器长度是无限的。所有这些听起来过于复杂，从我们在这里考虑所有可能的边界情况的意义上来说确实如此。在实践中，长度范围几乎总是与连接到空间腱的肌肉执行器一起使用，并且模型中将定义关节限制，有效地限制肌肉执行器的长度。如果您在这样的模型中遇到收敛错误，最可能的解释是您忘记包含关节限制。

### 状态执行器

如"计算"章节的执行模型部分所述，MuJoCo 支持具有内部动力学的执行器，其状态称为"激活"。

#### 激活限制

状态执行器的一个有用应用是"积分速度"执行器，由 `intvelocity` 快捷方式实现。与纯速度执行器（对传动目标的速度实现直接反馈）不同，积分速度执行器将积分器与位置反馈执行器耦合。在这种情况下，激活状态的语义是"位置执行器的设定点"，控制信号的语义是"位置执行器设定点的速度"。请注意，在真实机器人系统中，这种积分速度执行器是具有速度语义的执行器的最常见实现，而不是对速度的纯反馈，后者通常相当不稳定（在现实生活和仿真中都是如此）。

对于积分速度执行器，经常希望钳制激活状态，因为否则位置目标将继续积分超出关节限制，导致失去可控性。要查看激活钳制的效果，请加载下面的示例模型：
...
请注意，actrange 属性总是以原生单位（弧度）指定，即使关节范围可以是度（默认）或弧度，这取决于 compiler/angle 属性。

### 肌肉

我们提供了一套建模生物肌肉的工具。希望以最少努力添加肌肉的用户可以在执行器部分用一行 XML 来实现：
...
生物肌肉看起来彼此差异很大，但一旦应用某种缩放，它们的行为就会惊人地相似。我们的默认设置应用了这种缩放，这就是为什么人们可以在不调整任何参数的情况下获得合理的肌肉模型。当然，构建更详细的模型需要参数调整，如本节所解释的。

请记住，尽管肌肉模型相当复杂，它仍然是一种 MuJoCo 执行器类型，遵循与所有其他执行器相同的约定。肌肉可以使用 general 定义，但快捷方式 muscle 更方便。与所有其他执行器一样，力产生机制和传动是独立定义的。尽管如此，肌肉只有在连接到腱或关节传动时才具有（生物）物理意义。为了具体起见，我们这里假设腱传动。

首先我们讨论长度和长度缩放。传动（即 MuJoCo 腱）的可行长度范围将发挥重要作用；参见上面的长度范围部分。在生物力学中，肌肉和腱串联连接形成肌肉-腱执行器。我们的约定有所不同：在 MuJoCo 中，具有空间属性（特别是长度和速度）的实体是腱，而肌肉是拉动腱的抽象力生成机制。因此，MuJoCo 中的腱长度对应于生物力学中的肌肉+腱长度。我们假设生物腱是非弹性的，长度恒定为 $L_T$，而生物肌肉长度 $L_M$ 随时间变化。MuJoCo 腱长度是生物肌肉和腱长度的总和：

$$actuator\_length = L_T + L_M$$

另一个重要常数是肌肉的最佳静息长度，记为 $L_0$。它等于肌肉在零速度下产生最大主动力时的长度 $L_M$。我们不要求用户直接指定 $L_0$ 和 $L_T$，因为考虑到腱路径和包裹的空间复杂性，很难知道它们的数值。相反，我们自动计算 $L_0$ 和 $L_T$，如下所示。上述长度范围计算已经为 $L_T + L_M$ 提供了操作范围。此外，我们要求用户指定肌肉长度 $L_M$ 的操作范围，按（仍然未知的）常数 $L_0$ 缩放。这通过属性 range 完成；默认缩放范围是 $(0.75, 1.05)$。现在我们可以计算两个常数，使用实际范围和缩放范围必须彼此映射的事实：

$$(actuator\_lengthrange[0] - L_T) / L_0 = range[0]$$
$$(actuator\_lengthrange[1] - L_T) / L_0 = range[1]$$

在运行时，我们计算缩放的肌肉长度和速度为：

$$L = (actuator\_length - L_T) / L_0$$
$$V = actuator\_velocity / L_0$$

缩放量的优势在于所有肌肉在该表示中表现相似。该行为由许多实验论文中测量的力-长度-速度（$FLV$）函数捕获。我们将此函数近似如下：

![](https://mujoco.readthedocs.io/en/stable/_images/musclemodel.png)

函数的形式为：

$$FLV(L, V, act) = F_L(L) \cdot F_V(V) \cdot act + F_P(L)$$

与 MuJoCo 执行器的一般形式比较，我们看到 $F_L \cdot F_V$ 是执行器增益，$F_P$ 是执行器偏置。$F_L$ 是作为长度函数的主动力，而 $F_V$ 是作为速度函数的主动力。它们相乘以获得总体主动力（注意由 act 缩放，这是执行器激活）。$F_P$ 是被动力，无论激活如何都始终存在。$FLV$ 函数的输出是缩放的肌肉力。我们将缩放力乘以肌肉特定常数 $F_0$ 以获得实际力：

$$actuator\_force = -FLV(L, V, act) \cdot F_0$$

负号是因为正肌肉激活产生拉力。常数 $F_0$ 是零速度下的峰值主动力。它与肌肉厚度（即生理横截面积或 PCSA）相关。如果已知，可以用元素 muscle 的属性 force 设置。如果未知，我们将其设置为 $-1$，这是默认值。在这种情况下，我们依赖于较大肌肉往往作用于移动更多重量的关节这一事实。属性 scale 定义了这种关系：

$$F_0 = scale / actuator\_acc0$$

量 $actuator\_acc0$ 由模型编译器预计算。它是作用在执行器传动上的单位力引起的关节加速度的范数。直观地，$scale$ 确定肌肉"平均"有多强，而其实际强度取决于整个模型的几何和惯性属性。

到目前为止，我们遇到了定义单个肌肉属性的三个常数：$L_T, L_0, F_0$。此外，函数 $FLV$ 本身有几个参数，如上图所示：$l_{min}, l_{max}, v_{max}, f_{pmax}, f_{vmax}$。这些应该对所有肌肉都相同，但不同的实验论文建议不同的 FLV 函数形状，因此熟悉该文献的用户可能想要调整它们。我们提供了 MATLAB 函数 FLV.m，用于生成上图并显示我们如何计算 $FLV$ 函数。

在着手设计更准确的 $FLV$ 函数之前，请考虑这样一个事实：肌肉的操作范围比 $FLV$ 函数的形状影响更大，在许多情况下这个参数是未知的。下面是一个图形说明：

![](https://mujoco.readthedocs.io/en/stable/_images/musclerange.png)

这种图形格式在生物力学文献中很常见，显示每个肌肉的操作范围叠加在标准化的 $FL$ 曲线上（忽略垂直位移）。我们的默认范围用黑色显示。蓝色曲线是两个手臂肌肉的实验数据。人们可以找到具有小范围、大范围、跨越 $FL$ 曲线上升部分的范围或下降部分或两者兼而有之的肌肉。现在假设您有一个包含 50 块肌肉的模型。您是否相信有人做了仔细的实验并测量了模型中每块肌肉的操作范围，考虑到肌肉跨越的所有关节？如果没有，那么最好将肌肉骨骼模型视为具有与生物系统相同的一般行为，同时在各种细节上有所不同——包括某些研究社区非常感兴趣的细节。对于建模者认为是常数和已知的大多数肌肉属性，都有实验论文显示它们在某些条件下会变化。这不是为了阻止人们构建准确的模型，而是为了阻止人们过于相信他们的模型。

回到我们的肌肉模型，有肌肉激活 act。这是一阶非线性滤波器的状态，其输入是控制信号。滤波器动力学为：

$$\frac{\partial}{\partial t} act = \frac{ctrl - act}{\tau(ctrl, act)}$$

即使执行器没有指定控制范围，内部控制信号也被钳制到 [0, 1]。有两个时间常数用属性 timeconst 指定，即 $timeconst = (\tau_{act}, \tau_{deact})$，默认值为 $(0.01, 0.04)$。根据 [Millard 等人（2013）](https://doi.org/10.1115/1.4023390)，有效时间常数 $\tau$ 然后在运行时计算为：

$$\tau(ctrl, act) = \begin{cases}
\tau_{act} \cdot (0.5 + 1.5 \cdot act) & ctrl - act > 0 \\
\tau_{deact} / (0.5 + 1.5 \cdot act) & ctrl - act \leq 0
\end{cases}$$

由于上述方程描述了不连续切换，这在使用基于导数的优化时可能不受欢迎，我们引入了可选的平滑参数 tausmooth。当大于 0 时，切换被 mju_sigmoid 替换，它将在范围 $(ctrl - act) \pm tausmooth/2$ 内平滑插值两个值。

现在我们总结用户可能想要调整的元素 muscle 的属性，这取决于他们对生物力学文献的熟悉程度以及关于特定模型的详细测量的可用性：

**默认值**

> 在任何地方使用内置默认值。您所要做的就是将肌肉连接到腱，如本节开头所示。这产生了一个通用但合理的模型。

**scale**

> 如果您不知道单个肌肉的强度但想让所有肌肉更强或更弱，请调整 scale。这可以为每个肌肉单独调整，但在 default 元素中设置一次更有意义。
...
> 缩放长度中肌肉的操作范围在某些论文中也有提供。尚不清楚这种测量有多可靠（考虑到肌肉作用于许多关节），但它们确实存在。请注意范围在肌肉之间差异很大。

**timeconst**

> 肌肉由慢肌纤维和快肌纤维组成。典型肌肉是混合的，但一些肌肉具有更高比例的一种或另一种纤维类型，使它们更快或更慢。这可以通过调整时间常数来建模。$FLV$ 函数的 vmax 参数也应相应调整。

**tausmooth**

> 当为正时，平滑激活和去激活时间常数之间的过渡。虽然单个运动单元要么激活要么去激活，但整个肌肉将有许多单元的混合，导致相应的时间尺度混合。

**lmin, lmax, vmax, fpmax, fvmax**

> 这些是控制 $FLV$ 函数形状的参数。高级用户可以尝试它们；请参阅 MATLAB 函数 FLV.m。与 scale 设置类似，如果您想为所有肌肉更改 $FLV$ 参数，请在 default 元素中执行此操作。

**自定义模型**

> 用户可以实现不同的模型，而不是调整我们肌肉模型的参数，方法是将 general 执行器的 gaintype、biastype 和 dyntype 设置为"user"并在运行时提供回调。或者，将其中一些类型设置为"muscle"并使用我们的模型，同时替换其他组件。请注意，腱几何计算仍由标准 MuJoCo 流水线处理，为用户的肌肉模型提供 actuator_length、actuator_velocity 和 actuator_lengthrange 作为输入。然后自定义回调可以模拟弹性腱或我们选择省略的任何其他细节。

#### 与 OpenSim 的关系

生物力学研究人员使用的标准软件是 OpenSim。我们设计的肌肉模型在可能的情况下与 OpenSim 模型相似，同时进行简化以实现显著更快、更稳定的仿真。为了帮助 MuJoCo 用户转换 OpenSim 模型，这里我们总结相似性和差异。

激活动力学模型与 OpenSim 完全相同，包括默认时间常数。

$FLV$ 函数不完全相同，但 MuJoCo 和 OpenSim 都近似相同的实验数据，因此它们非常接近。有关 OpenSim 模型的描述和相关实验数据的总结，请参阅 Millard 等人（2013）。

我们假设非弹性腱，而 OpenSim 可以建模腱的弹性。我们决定在这里不这样做，因为腱弹性需要快速平衡假设，这反过来需要各种调整并且容易出现仿真不稳定性。在实践中，腱相当刚硬，它们的影响可以通过拉伸对应于非弹性情况的 $FL$ 曲线来近似捕获（Zajac（1989））。这可以在 MuJoCo 中通过缩短肌肉操作范围来实现。

羽状角（即肌肉与力线之间的角度）在 MuJoCo 中未建模，假设为 0。这种效果可以通过按比例缩小肌肉力并调整操作范围来近似。

腱包裹在 MuJoCo 中也更加有限。我们允许球体和无限圆柱体作为包裹对象，并要求两个包裹对象在腱路径中由固定位点分隔。这是为了避免腱路径的迭代计算需求。我们还允许将"侧位点"放置在球体或圆柱体内部，这会导致反向包裹：腱路径被约束穿过对象而不是绕过对象。这可以替代 OpenSim 中使用的环形包裹对象，以保持腱路径在给定区域内。总的来说，腱包裹是将 OpenSim 模型转换为 MuJoCo 模型的最具挑战性的部分，需要一些手动工作。从积极的方面来看，使用中的高质量 OpenSim 模型数量较少，因此一旦转换完成，我们就完成了。

下面我们说明了可用的四种腱包裹类型。请注意，包裹腱的弯曲部分呈现为直线，但几何管道使用实际曲线并分析计算它们的长度和力矩：

![](https://mujoco.readthedocs.io/en/stable/_images/tendonwraps.png)
