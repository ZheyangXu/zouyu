...
本节将介绍用于求解上述凸优化问题的数值算法（或称"求解器"）。牛顿法和共轭梯度法（CG）求解器基于约简原问题（8）工作，而投影高斯-赛德尔法（PGS）求解器则基于对偶问题（9）工作。需要注意的是，数值求解器仅在正向动力学中使用，而逆向动力学则可通过解析方法直接处理。

每种求解算法均可用于金字塔形和椭圆形摩擦锥，并支持约束雅可比矩阵及相关矩阵的稠密和稀疏表示。

CG（共轭梯度法）

> 该算法采用非线性共轭梯度法，结合Polak-Ribiere-Plus公式。线搜索采用一维牛顿法精确求解，并使用解析二阶导数。

Newton（牛顿法）

> 该算法实现了精确牛顿法，使用解析二阶导数和Cholesky分解计算Hessian矩阵。线搜索部分与CG方法相同。这是目前的默认求解器。

PGS（投影高斯-赛德尔法）

> 这是物理模拟器中最常用的算法，曾是MuJoCo的默认求解器，直到我们开发出在各方面似乎都更优的牛顿法。PGS使用对偶问题形式。与沿斜向方向改进解的梯度法不同，高斯-赛德尔法一次处理一个标量分量，并根据当前其他分量的值将其设为最优值。PGS的一次迭代在计算复杂度上相当于一次矩阵-向量乘法（尽管常数因子较大）。它具有一阶收敛特性，但仍能在几次迭代中取得显著进展。

当使用金字塔形摩擦锥时，问题涉及PGS传统上处理的盒约束。若直接将PGS应用于椭圆形摩擦锥产生的锥约束，它会陷入局部极小值连续体中（见左图）。这是因为PGS只能沿坐标轴方向取得进展。右图展示了我们解决此问题的方法。我们仍然一次更新一个接触点，但在每个接触点内，我们沿着适应约束表面的非正交轴进行更新：首先，沿着从锥尖到当前解的射线优化二次代价；然后，用一个通过当前解并垂直于接触法线的超平面切割锥体，得到一个椭球体（根据我们的接触模型，最高可达5维）。接下来，我们在此椭球体内优化二次代价。这是一个二次约束二次规划（QCQP）问题。由于只有一个标量约束（无论其非线性程度如何），对偶问题是关于未知拉格朗日乘子的标量优化问题。我们使用牛顿法求解此问题直至收敛——实际中通常不超过10次迭代，且涉及小型矩阵。总体而言，该算法对金字塔形锥的表现与PGS类似，但能够直接处理椭圆形锥而无需近似。虽然每个接触点需要更多计算，但接触维度更小，这两个因素大致平衡。

### 参数

这里我们将解释如何根据模型参数计算量 $R$ 和 $a_{ref}$。为使所选参数化方法有意义，我们首先需要理解这些量如何影响动力学。我们关注方程（9）中无约束最小化器：

$$f^+ = (A+R)^{-1}(a_{ref}-a_0)$$

如果恰好 $f^+ \in \Omega$，则 $f^+ = f$ 即为我们模型产生的实际约束力。我们关注这种情况，因为它很常见——在任意时刻，$\Omega$ 中的活动约束子集通常很小，而且这是我们能够实际分析的唯一情况。将 $f^+$ 代入约束动力学方程（10）并重新整理，得到：

$$a_1 = A(A+R)^{-1}a_{ref} + R(A+R)^{-1}a_0$$

因此，约束加速度在无约束加速度和参考加速度之间进行插值。特别地，在极限 $R \to 0$ 时，我们得到硬约束，且 $a_1 = a_{ref}$；而在极限 $R \to \infty$ 时，我们得到无限软的约束（即无约束），且 $a_1 = a_0$。因此，引入一个直接控制插值的模型参数是很自然的。我们称此参数为阻抗，用 $d$ 表示。它是一个维度为 $n_C$ 的向量，满足 $0 < d < 1$（按元素计算）。一旦指定了阻抗，我们可以计算正则化器的对角元素：

$$R_{ii} = \frac{1-d_i}{d_i}\hat{A}_{ii}$$

注意，我们使用的不是实际 $A$ 矩阵的对角线，而是其近似值。这是因为我们不希望在稀疏求解器或逆向动力学中计算 $A$。该近似值（仅限于对角线）是使用模型处于初始配置（mjModel.qpos0）时所有刚体、关节和肌腱的"末端执行器"惯性构建的。这些量由编译器计算。如果我们的近似恰好准确，且 $A$ 本身恰好是对角矩阵，那么每个标量约束的加速度将满足：

$$a_{1, i} = d_i a_{ref, i} + (1-d_i)a_{0, i}$$

这样我们就能实现期望的插值效果。虽然在一般情况下这并不完全成立，但我们的目标是构建一个合理且直观的约束模型参数化方法，并获得正确的比例关系。

接下来，我们解释如何计算参考加速度。如前所述，我们使用弹簧-阻尼器模型，通过阻尼和刚度系数按元素参数化：

$$a_{ref, i} = -b_i(Jv)_i - k_i r_i$$

回顾一下，$r$ 是位置残差（对于摩擦损失和椭圆锥的摩擦维度为零），而 $Jv$ 是投影到约束空间的关节速度；下标表示投影速度向量的一个分量。

总结来说，用户指定阻抗系数向量 $0 < d < 1$、阻尼系数 $b > 0$ 和刚度系数 $k > 0$。然后，MuJoCo按上述方式计算 $R$ 和 $a_{ref}$，并应用选定的优化算法求解问题（9）。如《建模》章节的求解器参数部分所述，MuJoCo提供了额外的自动化方法来设置 $d$、$b$ 和 $k$，以实现临界阻尼，或通过随距离变化的 $d$ 来模拟软接触层。

### 摩擦锥

如上所述，MuJoCo支持椭圆形摩擦锥和其金字塔形近似；所选求解器决定使用哪种摩擦锥类型。金字塔形近似有 $2(n-1)$ 条边，其中 $n$ 是由condim指定的接触空间维度。我们可以添加更多边以获得更好的椭圆锥近似，但这没有意义，因为结果求解器会比椭圆形对应物更慢。

人们可能预期，如果增加金字塔形近似中的边数，我们优化问题（7）的解会收敛到椭圆锥的解。这在硬接触极限下是正确的。然而，对于软接触，事实证明并非如此。这一令人惊讶的事实不仅仅是数学上的好奇现象；它可能对动力学产生可见影响，在MuJoCo早期版本中，这使得用金字塔形近似难以实现精细抓取行为。要理解这一现象，考虑固定问题（7）中的加速度变量 $x$ 并优化变形变量 $y$。可以证明，关于 $x$ 的结果优化问题等同于约束优化的惩罚方法，其中惩罚是从约束边界开始的半二次函数。可以将其想象为由边界投下的"阴影"。无论近似多么精确，椭圆锥及其金字塔形近似的这种阴影形状都是不同的。下图说明了这种效果，以2D接触为例，其中金字塔甚至不是近似，而是表示与椭圆锥相同的约束集。我们绘制了金字塔形（红色）和椭圆形（蓝色虚线）锥体的惩罚/阴影等值线，摩擦系数从左到右变化。从数学上讲，金字塔形情况下的惩罚是二次样条，而椭圆形情况下的惩罚包含二次函数减去二次函数的平方根的部分——允许在锥尖周围形成圆形等值线。

![](https://mujoco.readthedocs.io/en/stable/_images/softcontact.png)

总之，椭圆形和金字塔形摩擦锥定义了不同的软接触动力学（尽管它们通常非常接近）。椭圆模型更加原理化，更符合物理直觉，相应的求解器相当高效，但根据模型不同，可能不如金字塔形求解器那么高效。

## 碰撞检测

碰撞检测作用于几何体（geom），这些几何体刚性地附着在基础刚体上。碰撞检测的输出是活动接触列表，定义为接触距离小于其边界参数的接触。它们存储在全局数组mjData.contact中，然后用于构建约束雅可比矩阵并计算约束力。下面我们将解释如何选择几何体对进行碰撞检查，如何进行碰撞检查，以及如何确定结果接触的参数。

### 选择

如果模型有 $n$ 个几何体，则有 $n(n-1)/2$ 个可能碰撞的几何体对。对所有这些对进行详细检查（也称为近相碰撞检测）对大型系统来说计算成本过高。幸运的是，用户在建模阶段可以排除一些不需要的潜在碰撞，而其他碰撞则可以在不进行详细检查的情况下快速剔除。MuJoCo提供了灵活的机制来决定哪些几何体对需要详细检查。决策过程包括两个阶段：生成和过滤。

生成

> 首先，我们通过合并两个来源生成候选几何体对列表：可能包含碰撞几何体的刚体对，以及在MJCF中使用pair元素定义的显式几何体对列表。

> 刚体对是通过基于改进的扫描排序算法的广相碰撞检测生成的。该改进是选择所有几何体中心协方差矩阵的主特征向量作为排序轴，以最大化分布。然后，对于每个刚体对，使用轴对齐边界框（AABB）的静态边界体积层次结构（BVH二叉树）执行中相碰撞检测。每个刚体都配备了其几何体的AABB树，对于所有内部节点或叶节点，分别与刚体惯性或几何体框架对齐。

> 最后，用户可以使用MJCF中的exclude元素显式排除某些刚体对。在此步骤结束时，我们得到的几何体对列表通常比 $n(n-1)/2$ 小得多，但在进行详细碰撞检查前仍可进一步剔除。

过滤

> 接下来，我们对上一步生成的列表应用四个过滤器。过滤器1和2适用于所有几何体对。过滤器3和4仅适用于由刚体对机制生成的对，从而允许用户通过显式指定几何体对来绕过这些过滤器。

> 1. 两个几何体的类型必须对应于能够执行详细检查的碰撞函数。这通常是满足的，但有例外情况（例如，不支持平面-平面碰撞），此外，用户可以用NULL指针覆盖默认碰撞函数表，有效地禁用特定几何体类型之间的碰撞。
> 2. 应用边界球体测试，考虑接触边界。如果对中的一个几何体是平面，这将变为平面-球体测试。
> 3. 两个几何体不能属于同一刚体。此外，它们不能属于父子刚体关系，除非父刚体是世界刚体。这样做的目的是避免刚体和关节内的永久接触。请注意，如果几个刚体焊接在一起（即它们之间没有关节），则在此测试中视为单个刚体。用户可以禁用父过滤器测试，但不能禁用同一刚体测试。
> 4. 两个几何体必须在以下意义上"兼容"。每个几何体都有整数参数contype和conaffinity。下面的布尔表达式必须为真才能通过测试：

> `` `(contype1 & conaffinity2) || (contype2 & conaffinity1)` ``

> 这要求一个几何体的contype和另一个几何体的conaffinity有一个共同的位设置为1。这是一种强大的机制，借鉴自Open Dynamics Engine。所有几何体的默认设置是contype = conaffinity = 1，这总是通过测试，因此如果用户一开始感到困惑，可以忽略这一机制。

### 检查

详细碰撞检查（也称为近相或窄相碰撞检测）由取决于对中几何体类型的函数执行。窄相碰撞函数表可以在engine_collision_driver.c顶部查看，并向希望安装自己碰撞检测器的用户公开为mjCOLLISIONFUNC。MuJoCo支持多种基本几何形状：平面、球体、胶囊体、圆柱体、椭球体和盒体。它还支持三角形网格和高度场。

除了SDF插件（见其文档）外，碰撞检测仅限于凸几何体。所有基本类型都是凸的。高度场不是凸的，但在内部被视为三角形棱柱的集合（使用超出上述过滤器的自定义碰撞剔除）。用户指定的网格可以是非凸的，并按原样渲染。但是，对于碰撞目的，它们被替换为凸包（在模拟器中使用'H'键可视化），由qhull库计算。

#### 凸碰撞

所有不具有解析碰撞检测器的几何体对（如网格）的碰撞，均由以下两种通用凸碰撞检测（CCD）管道之一处理：

**原生管道（默认）**

> 原生CCD管道（"nativeccd"）在MuJoCo中原生实现，基于Gilbert-Johnson-Keerthi和扩展多面体算法（GJK / EPA）。原生管道比基于MPR的管道更快、更稳健。

**libccd管道（传统）**

> 这一传统管道基于libccd库，使用Minkowski门户细化（MPR）。通过禁用nativeccd标志可以激活它。

两种管道都由公开为mjOption.ccd_tolerance（ccd_tolerance）和mjOption.ccd_iterations（ccd_iterations）的容差（以距离为单位）和最大迭代参数控制。

#### 多接触点

某些碰撞检测器可以为每对碰撞返回多个接触点，以模拟线接触或面接触，如两个平面物体接触时。例如，胶囊体-平面和盒体-平面碰撞检测器分别最多可返回两个或四个接触点。标准通用凸碰撞算法如MPR和GJK总是返回单个接触点，这对面接触场景（如盒体堆叠）是有问题的。MuJoCo的两种CCD管道都能为每对接触返回多个点（"multiccd"）。这一行为由multiccd标志控制，但在不同管道中实现方式不同，各有优缺点：

**libccd管道（传统）**

> 通过围绕切线轴将两个几何体旋转±1e-3弧度并重新运行碰撞例程来查找多个接触点。如果检测到新接触，则添加它，最多允许4个额外接触点。这种方法有效，但将每次碰撞调用的成本增加了5倍。

**原生管道**

> 原生multiccd通过对解处的接触表面进行新颖分析来发现多个接触，避免了碰撞例程的完全重新运行，因此实际上是"免费"的。请注意，原生multiccd目前不支持正接触边界。如果两个几何体中的一个具有正边界，原生multiccd将回退到传统算法。

#### 几何体距离

窄相碰撞函数驱动mj_geomDistance函数和相关碰撞传感器。由于MPR的限制，传统管道将返回不正确的值（上图），除非在相对于几何体大小的非常小的距离处，因此不建议用于此用例。相比之下，基于GJK的原生管道（下图）在所有距离上都能计算正确值。

![](https://mujoco.readthedocs.io/en/stable/_images/ccd_light.gif)

#### 凸分解

为了模拟除高度场外的非凸物体，用户必须将其分解为凸几何体（可以是基本形状或网格）的并集，并将它们附加到同一刚体上。高度场本质上是一种自动分解为棱柱的形状。

可以在MuJoCo外部使用开源网格分解工具（如CoACD库）来自动完成此过程。最后，所有内置碰撞函数都可以替换为自定义回调。例如，这可用于集成通用的"三角形汤"碰撞检测器。但我们不推荐这种方法。预处理几何体并将其表示为凸几何体的并集需要一些工作，但这在运行时会带来回报，产生更快、更稳定的模拟。

例外情况是SDF插件（见其文档），在某些情况下可能会很高效，但有其他要求和限制。

## 模拟管道

这里我们总结正向和逆向动力学中涉及的计算序列。其中大部分已经描述过。请记住，mjModel.opt.disableflags和mjModel.opt.enableflags中的位标志可用于跳过默认步骤和启用可选步骤。此处未显示回调。

### 正向动力学

源文件engine_forward.c包含高级正向动力学管道。

#### 顶层

顶层函数mj_step调用下面的整个计算序列。
* mj_forward仅调用第2-22阶段，计算连续时间正向动力学，以加速度mjData.qacc结束。
* mj_step1调用第1-18阶段，mj_step2调用第19-25阶段，将mj_step分为两个不同的阶段。这允许用户编写依赖于位置和速度导出量的控制器（但不依赖于力，因为这些尚未计算）。请注意，mj_step1 → mj_step2管道不支持Runge Kutta积分器。
* mj_fwdPosition调用第2-11阶段，即管道中依赖位置的部分。

#### 阶段

下面我们描述管道阶段和对应每个阶段的API函数。所有函数都将其输出写入mjData的属性中。将下面的列表与mjData结构定义进行比较是很有启发性的，其中属性块上方的注释指定了计算它们的函数。请注意，每个阶段都依赖于在一些或所有先前阶段中计算的值。

01. 检查位置和速度是否有无效或不可接受的大实数值，表明发散。如果检测到发散，将自动重置状态并引发相应的警告：`mj_checkPos`、`mj_checkVel`

**位置**
下面的阶段计算依赖于广义位置mjData.qpos的量。

02. 计算前向运动学。这产生所有刚体、几何体、站点、相机和灯光的全局位置和方向。它还对所有四元数进行归一化：mj_kinematics、mj_camlight

03. 计算刚体惯性和关节轴，在以相应运动学子树质心为中心的全局坐标系中：mj_comPos

04. 计算与柔性物体相关的量：mj_flex

05. 计算肌腱长度和力臂。这包括计算空间肌腱的最小长度路径：mj_tendon

06. 计算复合刚体惯性和关节空间惯性矩阵：mj_makeM

07. 计算关节空间惯性矩阵的稀疏分解：mj_factorM

08. 构建活动接触列表。这包括广相和近相碰撞检测：mj_collision

09. 构建约束雅可比矩阵并计算约束残差：mj_makeConstraint

10. 计算执行器长度和力臂：mj_transmission

11. 计算约束求解器所需的矩阵和向量：mj_projectConstraint

12. 计算仅依赖于位置的传感器数据，如果启用，计算势能：mj_sensorPos、mj_energyPos

**速度**

下面的阶段计算依赖于广义速度mjData.qvel的量。由于管道的顺序依赖结构，实际依赖是qpos和qvel。

13. 计算肌腱、柔性边缘和执行器速度：mj_fwdVelocity

14. 计算刚体速度和关节轴变化率，同样在以子树质心为中心的全局坐标系中：mj_comVel

15. 计算被动力——关节和肌腱中的弹簧阻尼器以及流体力：mj_passive

16. 计算依赖于速度的传感器数据，如果启用，计算动能（如果传感器需要，调用mj_subtreeVel）：mj_sensorVel

17. 计算参考约束加速度：mj_referenceConstraint

18. 计算科里奥利力、离心力和重力向量：mj_rne

**力/加速度**

下面的阶段计算依赖于用户输入的量。由于管道的顺序性质，实际依赖是整个积分状态。

19. 计算执行器力和激活动力学（如果定义）：mj_fwdActuation

20. 计算由除（尚未知道的）约束力之外的所有力产生的关节加速度：mj_fwdAcceleration

21. 使用选定的求解器计算约束力，并更新关节加速度以考虑约束力。这产生向量mjData.qacc，它是正向动力学的主要输出：mj_fwdConstraint

22. 如果启用，计算依赖于力和加速度的传感器数据（如果传感器需要，调用mj_rnePostConstraint）：mj_sensorAcc

23. 检查加速度是否有无效或不可接受的大实数值。如果检测到发散，将自动重置状态并引发相应的警告：mj_checkAcc

24. 比较正向和逆向动力学的结果，以诊断正向动力学中的求解器收敛不良。这是一个可选步骤，仅在启用时执行：mj_compareFwdInv

25. 使用选定的积分器使模拟状态前进一个时间步长。请注意，Runge-Kutta积分器会再重复上述序列三次，但可选计算仅执行一次：mj_Euler、mj_RungeKutta或mj_implicit中的一个

#### mjData中的一致性

MuJoCo计算管道完全是命令式的，没有什么会自动发生。这导致的行为对更熟悉其他范式的用户来说可能显得意外。以下是两个可能令人惊讶但符合设计意图的行为示例：

01. 设置状态后，状态导出量不会自动与新状态对应。必须手动调用所需的阶段。例如，设置广义位置mjData.qpos后，没有首先调用mj_kinematics，笛卡尔位置和方向将与qpos不一致。

02. 在mj_step之后，它在更新状态后立即终止，mjData中的量对应于前一个状态（或更准确地说，是前一个和当前状态之间的过渡）。特别是，所有依赖位置的传感器值和依赖位置的计算（如运动学雅可比矩阵）都将基于前一个位置。

#### 可重现性

MuJoCo的模拟管道完全是确定性和可重现的——如果保存并重新加载轨迹中的状态并再次调用mj_step，结果的下一个状态将是相同的。但是，有一些重要的注意事项：

01. 保存所有必需的积分状态组件。特别是预热启动加速度对下一个状态只有很小的影响，但如果需要位级相等，应该保存。

02. 状态之间的任何数值差异，无论多小，在积分后都会变得显著，尤其是对于有接触的系统。接触事件具有高李雅普诺夫指数；这是任何刚体模拟器的特性（实际上也是现实世界物理的特性），并非MuJoCo特有。

03. 只有在单一版本、相同架构上才能保证完全可重现性。在版本发布之间，由于代码优化等原因，通常会出现小的数值差异。这意味着保存初始状态和开环控制序列时，在同一版本内展开的轨迹将完全相同，但在不同MuJoCo版本或不同操作系统之间可能会有所不同。

### 逆向动力学

顶层函数mj_inverse调用以下计算序列。上述关于一致性和可重现性的注意事项同样适用于此处。

01. 计算前向运动学。
02. 计算刚体惯性和关节轴。
03. 计算肌腱长度和力臂。
04. 计算执行器长度和力臂。
05. 计算复合刚体惯性并形成关节空间惯性矩阵。
06. 计算关节空间惯性矩阵的稀疏分解。
07. 构建活动接触列表。
08. 构建约束雅可比矩阵并计算约束残差。
09. 计算仅依赖于位置的传感器数据，如果启用，计算势能。
10. 计算肌腱和执行器速度。
11. 计算刚体速度和关节轴变化率。
12. 计算依赖于速度的传感器数据，如果启用，计算动能。如果传感器需要，调用mj_subtreeVel。
13. 计算所有被动力。
14. 计算参考约束加速度。
15. 如果设置了invdiscrete标志且积分器不是RK4，将输入加速度从离散时间转换为连续时间。
16. 计算约束力。这是通过解析完成的，无需使用数值求解器。
17. 计算非约束系统的逆向动力学。
18. 如果启用，计算依赖于力和加速度的传感器数据。如果传感器需要，调用mj_rnePostConstraint。
19. 通过组合所有结果计算向量mjData.qfrc_inverse。这是逆向动力学的主要输出。它等于外部力和执行力之和。

### 导数

MuJoCo的整个计算管道（包括其约束求解器）原则上都是解析可微的。编写这些导数的高效实现是开发团队的长期目标。平滑动力学（不包括约束）相对于速度的解析导数已经计算出来，并启用了两种隐式积分器。

请注意，求解器阻抗的默认值使得接触在默认情况下是不可微的，需要将其设置为0才能使接触力的onset变得平滑。

目前有两个可用函数，它们使用高效的有限差分来计算动力学雅可比矩阵：

**mjd_transitionFD**：

> 计算离散时间正向动力学（mj_step）的状态转移和控制转移雅可比矩阵。参见文档。

**mjd_inverseFD**：

> 计算连续或离散时间逆向动力学（mj_inverse）的雅可比矩阵。参见文档。

这些导数通过利用MuJoCo的可配置计算管道变得高效，使得在不需要时不会重新计算量。例如，当对控制进行差分时，仅依赖于位置和速度的量不会重新计算。此外，求解器预热启动、四元数和控制钳位都得到了正确处理。支持前向和中心差分。
