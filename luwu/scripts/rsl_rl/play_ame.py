...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Save attention weights during play.",
)
parser.add_argument(
    "--vis_attention",
    action="store_true",
    default=False,
    help="Visualize attention weights as colored markers (env 0).",
)
parser.add_argument(
...
parser.add_argument(
    "--task", type=str, default="AME-G1-29DOF-Play-v0", help="Name of the task."
)
parser.add_argument(
    "--agent",
    type=str,
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help=f"Manual Headless mode for convenient debug in IDE. \
...
def main(
    ...
    def _vis_attention_on_terrain(attn: torch.Tensor, env):
        ...
        try:
            ...
            if n_rays == n_attn:
                ...
                if scan_grid_shape is not None:
                    ...
                    if rows * cols == n_rays:
                        ...
                        if hasattr(policy_nn, "cnn_downsample") and bool(
                            getattr(policy_nn, "cnn_downsample")
                        ):
                ...
                if selected_hits is None:
                    sample_idx = (
                        torch.linspace(
                            0, n_rays - 1, steps=n_attn, device=ray_hits.device
                        )
                        .round()
                        .long()
                    )
            ...
            attn_norm = (attn_vals - attn_vals.min()) / (
                attn_vals.max() - attn_vals.min() + 1e-6
            )
            n_vis = selected_hits.shape[0]

            # Color bins (blue -> red)
            bins = (
                (attn_norm * (attention_num_bins - 1))
                .long()
                .clamp(0, attention_num_bins - 1)
            )
            ...
            attention_visualizer.visualize(
                points, orientations, marker_indices=bins, scales=scales
            )
    ...
    if len(attention_weights_list) > 0:
        ...
        np.save(
            os.path.join(PROJ_ROOT_DIR, "attention_weights.npy"),
            np.array(attention_weights_list),
        )
        print(
            f"[INFO] Attention weights saved to attention_weights.npy, shape: {np.array(attention_weights_list).shape}"
        )
