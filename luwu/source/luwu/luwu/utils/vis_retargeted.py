...
class RetargetedMotionLoader:
    def __init__(
        self, robot_name: str, motion_file: str, lab_joint_names: list[str]
    ) -> None:
        ...
        if not os.path.exists(self.motion_file):
            raise FileNotFoundError(
                f"Motion file {self.motion_file} does not exist, please check the file."
            )
        # yaml config file contains some retargeted data config
        self.config_file = os.path.join(motion_dir, "retargeted.yaml")
        if not os.path.exists(self.config_file):
            raise FileNotFoundError(
                f"Config file {self.config_file} does not exist, please check the file."
            )
    ...
    def _get_joint_mapping(self) -> dict:
        ...
        for joint_name in self.lab_joint_names:
            if joint_name not in self.retargeted_joint_names:
                raise ValueError(
                    f"Joint name {joint_name} not found in retargeted joint names."
                )
        for joint_name in self.retargeted_joint_names:
            if joint_name not in self.lab_joint_names:
                raise ValueError(
                    f"Joint name {joint_name} not found in lab joint names."
                )

        self.retargeted_to_lab_mapping = [
            self.retargeted_joint_names.index(joint_name)
            for joint_name in self.lab_joint_names
        ]
        self.lab_to_retargeted_mapping = [
            self.lab_joint_names.index(joint_name)
            for joint_name in self.retargeted_joint_names
        ]
    ...
    def _on_keyboard_event(self, event: carb.input.KeyboardEvent) -> None:
        ...
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            if event.input.name == "RIGHT":
                ...
                print(
                    f"[INFO]: Switched to previous motion: {self.current_motion_name}"
                )
...
def run_simulator(
    ...
    re_motion_loader = RetargetedMotionLoader(
        args_cli.robot, args_cli.motion_file, robot.data.joint_names
    )

    _, _, _, joint_pos_smpl, joint_pos_robot = re_motion_loader.get_motion_data(
        0, sim.device
    )
    ...
    marker_indices = torch.zeros(
        num_joint_smpl + num_joint_robot, dtype=torch.int32, device=sim.device
    )
    ...
    while simulation_app.is_running():
        ...
        # get the motion data
        root_pos, root_quat, dof_pos, joint_pos_smpl, joint_pos_robot = (
            re_motion_loader.get_motion_data(current_time, sim.device)
        )
