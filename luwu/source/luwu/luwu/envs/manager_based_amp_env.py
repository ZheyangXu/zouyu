...
from luwu.managers import AnimationManager, MotionDataManager, PreviewObservationManager
...
class ManagerBasedAmpEnv(ManagerBasedAnimationEnv):
    ...
    def __init__(
        self, cfg: ManagerBasedAmpEnvCfg, render_mode: str | None = None, **kwargs
    ):
    ...
    def _merge_terminal_obs(
        ...
        for key, value in current_obs.items():
            ...
            if isinstance(value, dict) and isinstance(preview_value, dict):
                terminal_obs[key] = self._merge_terminal_obs(
                    value, preview_value, reset_env_ids
                )
            elif isinstance(value, torch.Tensor) and isinstance(
                preview_value, torch.Tensor
            ):
    ...
    def _preview_terminal_obs(
        ...
        if hasattr(self.observation_manager, "preview_group"):
            ...
            for group_name in group_names:
                preview_obs[group_name] = self.observation_manager.preview_group(
                    group_name
                )
            return preview_obs
    ...
    def load_managers(self):
        ...
        self.observation_manager = PreviewObservationManager(
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        if terminal_obs_preview is not None:
            ...
            current_terminal_groups = {
                group_name: self.obs_buf[group_name]
                for group_name in terminal_obs_preview
            }
            terminal_obs = self._merge_terminal_obs(
                current_terminal_groups, terminal_obs_preview, reset_env_ids
            )
