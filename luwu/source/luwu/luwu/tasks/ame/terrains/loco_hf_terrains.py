...
def stones_bridge_terrain(
    difficulty: float, cfg: loco_hf_terrains_cfg.HfStonesBridgeTerrainCfg
) -> np.array:
    ...
    def place_column_pair(primary_pos: int, along_x: bool) -> None:
        if along_x:
            ...
            for sign in (-1, 1):
                ...
                cy = int(
                    np.clip(
                        center_y + sign * base_offset + jitter,
                        axis_limit_low,
                        axis_limit_high,
                    )
                )
            ...
            for sign in (-1, 1):
                ...
                cx = int(
                    np.clip(
                        center_x + sign * base_offset + jitter,
                        axis_limit_low,
                        axis_limit_high,
                    )
                )
...
@height_field_to_mesh
def concentric_gap_terrain(
    difficulty: float, cfg: loco_hf_terrains_cfg.HfConcentricGapTerrainCfg
) -> np.ndarray:
    """
    ...
    while (stop_x - start_x) > platform_width and (stop_y - start_y) > platform_width:
        if is_gap:
            ...
            # Fill ground ring
            hf_raw[start_x:stop_x, start_y:stop_y] = randint(
