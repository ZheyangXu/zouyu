...
def joint_coordination_rel(
    ...
    coord_signs: list[list[float]] = None,
) -> torch.Tensor:
    ...
    if (
        not hasattr(env, "joint_coord_joints_cache")
        or env.joint_coord_joints_cache is None
    ):
        # Cache joint positions for all pairs
    ...
    for i, joint_indices in enumerate(env.joint_coord_joints_cache):
        ...
        joint1_rel = (
            asset.data.joint_pos[:, joint1_idx]
            - asset.data.default_joint_pos[:, joint1_idx]
        )
        joint2_rel = (
            asset.data.joint_pos[:, joint2_idx]
            - asset.data.default_joint_pos[:, joint2_idx]
        )
