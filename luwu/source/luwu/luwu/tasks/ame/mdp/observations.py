...
def ray_hits_s(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    sensor_quat_expanded = (
        sensor_quat.unsqueeze(1).expand(N, B, 4).reshape(N * B, 4)
    ).to(torch.float)
...
def elevation_map(
    env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg, noise: bool = False
) -> torch.Tensor:
    ...
    if noise:
        ...
        if getattr(
            env, "_elevation_map_offset", None
        ) is None or env._elevation_map_offset.shape != (N, 1):
        ...
        if hasattr(env, "reset_buf"):
            ...
            if len(reset_env_ids) > 0:
                ...
                env._elevation_map_offset[reset_env_ids] = (
                    torch.rand((len(reset_env_ids), 1), device=env.device) * 0.1 - 0.05
                )
