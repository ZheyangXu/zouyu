...
import luwu.tasks.tracking.deepmimic.mdp as mdp
from luwu.envs import ManagerBasedAnimationEnvCfg
from luwu.managers import AnimationTermCfg as AnimTerm
from luwu.managers import MotionDataTermCfg as MotionDataTerm
...
class RewardsCfg:
    ...
    ref_track_root_pos_w_error_exp = RewTerm(
        func=mdp.ref_track_root_pos_w_error_exp, weight=1.0, params=MISSING
    )

    ref_track_quat_error_exp = RewTerm(
        func=mdp.ref_track_quat_error_exp, weight=0.5, params=MISSING
    )

    ref_track_root_vel_w_error_exp = RewTerm(
        func=mdp.ref_track_root_vel_w_error_exp, weight=0.1, params=MISSING
    )
    ...
    ref_track_dof_pos_error_exp = RewTerm(
        func=mdp.ref_track_dof_pos_error_exp, weight=0.5, params=MISSING
    )

    ref_track_dof_vel_error_exp = RewTerm(
        func=mdp.ref_track_dof_vel_error_exp, weight=0.1, params=MISSING
    )
...
class TerminationsCfg:
    ...
    deviation_key_body_pos_w = DoneTerm(
        func=mdp.deviation_key_body_pos_w, params=MISSING
    )
