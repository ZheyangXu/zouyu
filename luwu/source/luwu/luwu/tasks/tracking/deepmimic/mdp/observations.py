...
def key_body_pos_b(
    ...
    asset_cfg: SceneEntityCfg = SceneEntityCfg(
        "robot", body_names=MISSING, preserve_order=True
    ),
) -> torch.Tensor:
    robot: Articulation = env.scene[asset_cfg.name]
...
def ref_root_pos_error(
    ...
    pos_diff = ref_root_pos_w - root_pos_w.unsqueeze(
        1
    ...
    if abs_height:
        ...
        return pos_diff[:, :, :2].reshape(
            num_envs, -1
        )  # shape: (num_envs, num_steps * 2)
...
def ref_root_rot_tan_norm(
    ...
    ref_root_rotm = math_utils.matrix_from_quat(
        ref_root_quat
    )  # shape: (num_envs, num_steps, 3, 3)
    ref_root_tan_vec = ref_root_rotm[:, :, :, 0]  # shape: (num_envs, num_steps, 3)
    ref_root_norm_vec = ref_root_rotm[:, :, :, 2]  # shape: (num_envs, num_steps, 3)
    obs = torch.cat(
        [ref_root_tan_vec, ref_root_norm_vec], dim=-1
    )  # shape: (num_envs, num_steps, 6)
...
def ref_root_ang_vel_b(
    ...
    ref_root_ang_vel_w = (
        animation_term.get_root_ang_vel_w()
    )  # shape: (num_envs, num_steps, 3)
    ref_root_quat = animation_term.get_root_quat()  # shape: (num_envs, num_steps, 4)
...
def ref_key_body_pos_b(
    ...
    ref_key_body_pos_b = (
        animation_term.get_key_body_pos_b()
    )  # shape: (num_envs, num_steps, num_key_bodies, 3)
