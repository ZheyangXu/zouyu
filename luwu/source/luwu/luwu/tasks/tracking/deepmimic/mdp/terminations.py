...
def deviation_key_body_pos_b(
    ...
    dist = torch.norm(
        key_body_pos_b - ref_key_body_pos_b, dim=-1
    )  # shape: (num_envs, M)
...
def deviation_key_body_pos_w(
    ...
    key_body_pos_w = robot.data.body_pos_w[
        :, asset_cfg.body_ids, :
    ] - env.scene.env_origins.unsqueeze(
        1
    )  # shape: (num_envs, M, 3)
    ...
    ref_key_body_pos_w = ref_root_pos_w.unsqueeze(1) + math_utils.quat_apply(
        ref_root_quat.unsqueeze(1).expand(-1, num_key_bodies, -1).contiguous(),
        ref_key_body_pos_b,
    )

    dist = torch.norm(
        ref_key_body_pos_w - key_body_pos_w, dim=-1
    )  # shape: (num_envs, M)
    max_dist, _ = torch.max(dist, dim=-1)  # shape: (num_envs,)
    return max_dist > threshold
