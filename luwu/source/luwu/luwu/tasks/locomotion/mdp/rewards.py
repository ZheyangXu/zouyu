...
def stand_still(
    ...
    """
    Docstring for stand_still reward function.
    """
    asset: Articulation = env.scene[asset_cfg.name]
...
def upward(
    ...
    """Reward the agent for having its base pointing upwards."""
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
...
def joint_position_penalty(
    ...
    """
    Penalize the robot for moving its joints away from the default position
    when the commanded velocity is low.
    """
    asset: Articulation = env.scene[asset_cfg.name]
...
def feet_contact_without_cmd(
    ...
    reward = torch.sum(is_contact.float(), dim=1)
