...
class AnimationTerm(ManagerTermBase):
    ...
    def __init__(self, cfg: AnimationTermCfg, env: ManagerBasedAnimationEnv):
        ...
        for component in cfg.motion_data_components:
            ...
            setattr(
                self,
                f"{component}_buffer",
                torch.zeros(buffer_shape, device=env.device, dtype=torch.float32),
            )
        ...
        if self.cfg.enable_visualization:
            ...
            self.key_body_marker: VisualizationMarkers = VisualizationMarkers(
                marker_cfg
            )
    ...
    def reset(self, env_ids: Sequence[int] | None = None):
        ...
        if self.num_steps > 1:
            self.motion_fetch_time[env_ids, 1:] = (
                self.motion_fetch_time[env_ids, 0:1]
                + self.step_indices[1:].float() * self._env.step_dt
            )
    ...
    def update(self, dt: float):
        if self.cfg.random_fetch:
            ...
            if self.num_steps > 1:
                self.motion_fetch_time[:, 1:] = (
                    self.motion_fetch_time[:, 0:1] + self.step_indices[1:].float() * dt
                )
    ...
    def _visualize(self):
        ...
        if robot_anim is None:
            print(
                "[WARNING] AnimationTerm visualization: 'robot_anim' not found in the scene."
            )
            return
        ...
        key_body_pos_b = self.key_body_pos_b_buffer[
            :, 0, :, :
        ]  # (num_envs, num_key_bodies, 3)
