...
from luwu.utils.math import ang_vel_from_quat_diff, quat_slerp, vel_forward_diff
...
class MotionDataTerm(ManagerTermBase):
    ...
    def _load_motion_data(self):
        ...
        motion_files = [
            f for f in os.listdir(self.cfg.motion_data_dir) if f.endswith(".pkl")
        ]
        if not motion_files:
            raise ValueError(
                f"No motion data files with .pkl extension found in {self.cfg.motion_data_dir}."
            )
        ...
        for motion_name, motion_weight in self.motion_weights_dict.items():
            ...
            if not isinstance(motion_raw_data, dict):
                raise ValueError(
                    f"Motion data file {motion_file} does not contain a valid dictionary."
                )
            ...
            root_pos_w = (
                torch.from_numpy(motion_raw_data["root_pos"]).to(self.device).float()
            )
            ...
            root_quat = (
                torch.from_numpy(motion_raw_data["root_rot"]).to(self.device).float()
            )
            ...
            dof_pos = (
                torch.from_numpy(motion_raw_data["dof_pos"]).to(self.device).float()
            )
            ...
            key_body_pos_w = (
                torch.from_numpy(motion_raw_data["key_body_pos"])
                .to(self.device)
                .float()
            )
        ...
        self.motion_fps = torch.tensor(
            self.motion_fps, dtype=torch.float32, device=self.device
        )
        self.motion_dt = torch.tensor(
            self.motion_dt, dtype=torch.float32, device=self.device
        )
        self.motion_durations = torch.tensor(
            self.motion_durations, dtype=torch.float32, device=self.device
        )
        self.motion_num_frames = torch.tensor(
            self.motion_num_frames, dtype=torch.int32, device=self.device
        )
        self.motion_loop_modes = torch.tensor(
            self.motion_loop_modes, dtype=torch.int32, device=self.device
        )
        # Get the normalized motion weights
        self.motion_weights = torch.tensor(
            self.motion_weights, dtype=torch.float32, device=self.device
        )
    ...
    def sample_motions(self, n: int) -> torch.Tensor:
        ...
        motion_ids = torch.multinomial(
            self.motion_weights, num_samples=n, replacement=True
        )
        return motion_ids
    ...
    def get_motion_state(
        ...
        key_body_pos_w = torch.lerp(
            key_body_pos_w_0, key_body_pos_w_1, blend.unsqueeze(1)
        )
...
@torch.jit.script
def calc_phase(
    times: torch.Tensor, motion_duration: torch.Tensor, loop_mode: torch.Tensor
) -> torch.Tensor:
